...
vec4 TilingAndBlending(in sampler2D sampler, in vec2 uv, in ivec3 blockPos)
{
	...
	SquareGrid(blockUVFract, weights, tile1, tile2); //weight is not between 0 and 1 because uv is between 0 and 1 for whole texture
	...
	return value/W + mean;
	//vec2 modTile = vec2(mod(tile2, ivec2(10, 10)))/10.0;
	//return vec4(modTile.x, modTile.y, 0., 1.0);
	//return vec4(mod(uniqueID, 5.0)/5.0, 1.0);
	return vec4(fract(uvContent1*nbBlocks), 0.5, 1.);
}
